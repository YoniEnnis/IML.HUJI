...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(self.iterations_):
            ...
            miss_class_indexes = np.argwhere(y != pred)
            e = np.sum(self.D_[miss_class_indexes])  # TODO: y != pred
            self.weights_.append(0.5 * np.log(1/e - 1))
            self.D_ = self.D_ * np.exp(-y * self.weights_[i] * pred)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for i in range(T):
            sums = sums + self.weights_[i] * self.models_[i].predict(X)
        return np.sign(sums)
